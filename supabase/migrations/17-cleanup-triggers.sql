-- REMOVER GATILHOS ANTIGOS/CONFLITANTES
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS handle_new_user_trigger ON auth.users;
DROP TRIGGER IF EXISTS sync_user_trigger ON auth.users;

-- RESTAURAR FUNÇÃO CORRETA E ROBUSTA
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
DECLARE
  extracted_role text;
  final_role "Role";
BEGIN
  -- Extrair texto com segurança
  extracted_role := NEW.raw_user_meta_data->>'role';
  
  -- Atribuição manual segura
  IF extracted_role = 'TECHNICIAN' THEN
    final_role := 'TECHNICIAN'::"Role";
  ELSIF extracted_role = 'COMPANY' THEN
    final_role := 'COMPANY'::"Role";
  ELSIF extracted_role = 'ADMIN' THEN
    final_role := 'ADMIN'::"Role";
  ELSE
    final_role := 'EMPLOYEE'::"Role";
  END IF;

  INSERT INTO public."User" (id, email, name, role)
  VALUES (
    NEW.id,
    NEW.email,
    COALESCE(NEW.raw_user_meta_data->>'name', NEW.email),
    final_role
  );
  
  RETURN NEW;
EXCEPTION
  WHEN OTHERS THEN
    -- AVISO CRÍTICO: Em caso de erro, LOGAR mas NÃO FALHAR o registro no Auth.
    -- Isso permite que o usuário seja criado e possamos corrigir os dados depois.
    RAISE WARNING 'CRITICAL ERROR in handle_new_user: %', SQLERRM;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- RECRIAR O GATILHO OFICIAL
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user();
